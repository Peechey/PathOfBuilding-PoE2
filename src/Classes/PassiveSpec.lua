...
function PassiveSpecClass:AllocNode(node, altPath)
	...
	if #node.intuitiveLeapLikesAffecting > 0 then
		node.alloc = true
		node.allocMode = (node.ascendancyName or node.isKeystone) and 0 or self.allocMode
		self.allocNodes[node.id] = node
	else
		for _, pathNode in ipairs(altPath or node.path) do
			pathNode.alloc = true
			pathNode.allocMode = (node.ascendancyName or node.isKeystone) and 0 or self.allocMode
